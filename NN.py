...
class SigmoidLayer(Layer):
    ...
    def forward(self, x):
        ...
        Arguments:
            x {np.ndarray} --).
 Input array of shape (batch_size, n_in
        Returns:
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        g = 1 / (1 + np.exp(-self._cache_current))
        return grad_z * g * (1 - g)
...
class ReluLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        self._cache_current[grad_z > 0] = 1
        self._cache_current[grad_z <= 0] = 0
        return grad_z
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
...
            - neurons {list} -- Number of neurons in each linear layer 
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        # start the list with the first layer
        self._layers = [LinearLayer(input_dim, neurons[0])]
        # add the activation function of the first layer, if it's not identity
        if self.activations[0] == "relu":
            self._layers.append(ReluLayer())
        elif self.activations[0] == "sigmoid":
            self._layers.append(SigmoidLayer())
        elif self.activations[0] == "identity":
            pass
    ...
    def forward(self, x):
        ...
        for layer in self._layers:
            input_output = layer.forward(input_output)
        return input_output
    ...
    def backward(self, grad_z):
        ...
        for layer in range(len(self._layers) - 1, -1, -1):
            input_output = self._layers[layer].backward(input_output)
        return input_output
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
...
class Trainer(object):
    ...
    def shuffle(input_dataset, target_dataset):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        shuffle = np.arange(len(input_dataset))
        np.random.shuffle(shuffle)
        return input_dataset[shuffle], target_dataset[shuffle]
    ...
    def train(self, input_dataset, target_dataset):
        ...
        for j in range(self.nb_epoch):
            ...
            mini_batch_number = len(input) // self.batch_size
            for b in range(mini_batch_number):
                mini_batch_input = input[b * self.batch_size: b * self.batch_size + self.batch_size]
                mini_batch_target = target[b * self.batch_size: b * self.batch_size + self.batch_size]
                input_split.append(mini_batch_input)
                target_split.append(mini_batch_target)
            # train the network
            for i in range(len(input_split)):
                # forward pass to get network prediction
                predictions = self.network.forward(input_split[i])
                # need the gradient to do the backward pass through the network. Get it by forward +
                # backward pass through the loss functions
                self._loss_layer.forward(predictions, target_split[i])
